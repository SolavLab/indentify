        ...
        if isfield(test{i},'obj_fun_val')
            if strcmp(override_obj_fun_val,'yes') %re-evaluate objFun
                if test{i}.runFlag==1
                    ...
                    test{i}.obj_fun_val = calcObjFun_anisotropic_indent(test{i},objectiveStruct); %evaluate objFun
                end
            end
        else
...
OF = 0.45*objectiveValues.Ff.sumOfSquares+0.35*objectiveValues.Fu_x.sumOfSquares+0.2*objectiveValues.Fu_y.sumOfSquares+0.00*objectiveValues.Fu_z.sumOfSquares;
% OF = 0*objectiveValues.Ff.sumOfSquares+0.33*objectiveValues.Fu_x.sumOfSquares+0.33*objectiveValues.Fu_y.sumOfSquares+0.34*objectiveValues.Fu_z.sumOfSquares;
% OF = objectiveValues.Ff.sumOfSquares;
...
xlabel(varried_parameters{operateOn(1)}, 'FontSize', fontSize) % label the x-axis with the name of the varried parameter
ylabel(varried_parameters{operateOn(2)}, 'FontSize', fontSize) % label the y-axis with the name of the varried parameter

h = figure('Units', 'centimeters', 'Position', [0, 0, 14, 6]); % create another figure
...
max_Z = max(local_Z, [], 'all');
min_Z = min(local_Z, [], 'all');

contour_lines = linspace(min_Z, max_Z, numCurves);
[~, std_idx] = min(abs(contour_lines - stdDeviation)); % Find the index of the value closest to S
[~, cont_idx] = min(abs(local_Z - stdDeviation),[],'all'); % Find the index of the point closest to S
contour_lines(std_idx) = stdDeviation;
contour_lines(std_idx:end) = stdDeviation:(3*abs(grad_temp(cont_idx))):(stdDeviation+(numCurves-std_idx)*3*abs(grad_temp(cont_idx)));
contour_lines(1:std_idx-1) = linspace(min_Z,stdDeviation,std_idx-1);
if std_idx==1 contour_lines=[0,contour_lines]; std_idx=2; numCurves=numCurves+1; end
contour_lines = [contour_lines, logspace(log10(contour_lines(end)),log10(max_Z),10)];
...
%Optimize colorbar:
clim([0, (stdDeviation+(numCurves-std_idx)*3.2*abs(grad_temp(cont_idx)))])


hold on 
axis manual

scatter(1,1,6,'white','o','filled')

% Scatter simulation run values on plot
% scatter(xData,yData,5,[0.2 0.2 0.2],".") % scatter the data points in black
...
% Set the axes numbering fontsize to match the axis labels
ax = gca;
...
cbh.TickLabels{end} = ['>', cbh.TickLabels{end}];
