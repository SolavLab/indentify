...
for dir_ind = 1:numel(dir_name_carray)
    ...
    for t=1:nSteps
        T = strcat('t_',num2str(t));  % Define field name
        objectiveValues.Ff.(T)=F;
        objectiveValues.Fu_x.(T)=F;
        objectiveValues.Fu_y.(T)=F;
        objectiveValues.Fu_z.(T)=F;
    end
    ...
    for i=1:numel(test)     % run over tests
        ...
        for t=1:nSteps % Loop over the time steps
            T = strcat('t_',num2str(t));  % Define field name
            objectiveValues.Ff.(T)(i) = test{i}.obj_fun_val.Ff(t+1);
            objectiveValues.Fu_x.(T)(i) = test{i}.obj_fun_val.Fu_x(t+1);
            objectiveValues.Fu_y.(T)(i) = test{i}.obj_fun_val.Fu_y(t+1);
            objectiveValues.Fu_z.(T)(i) = test{i}.obj_fun_val.Fu_z(t+1);
        end
    end
    ...
    TF_temp = TF;
    for dim=1:ndims(X)
        F = objectiveValues.Ff.(T);
        idx = find(isnan(F));
        first = true;
        if ~isempty(idx)
            for t=1:nSteps
                T = strcat('t_',num2str(t));
                [objectiveValues.Ff.(T),TF_temp] = fillmissing(objectiveValues.Ff.(T),'linear',dim,'EndValues','nearest');
                objectiveValues.Fu_x.(T) = fillmissing(objectiveValues.Fu_x.(T),'linear',dim,'EndValues','nearest');
                objectiveValues.Fu_y.(T) = fillmissing(objectiveValues.Fu_y.(T),'linear',dim,'EndValues','nearest');
                objectiveValues.Fu_z.(T) = fillmissing(objectiveValues.Fu_z.(T),'linear',dim,'EndValues','nearest');
                if first
            ...
            for i=1:length(idx) %check all interpolated data and reset
                if isinf(objectiveValues.Ff.(T)(idx(i)))
                    for t=1:nSteps
                        T = strcat('t_',num2str(t));
                        objectiveValues.Ff.(T)(idx(i))=NaN;
                        objectiveValues.Fu_x.(T)(idx(i))=NaN;
                        objectiveValues.Fu_y.(T)(idx(i))=NaN;
                        objectiveValues.Fu_z.(T)(idx(i))=NaN;
                    end
                end
            end
        end
    ...
    n_accuracy = 2; % uncomment this line to ovverride automatic accuracy
    ...
    % Determine objective function shapes of each test (deepest indentation)
    Hf = getHessian(X_norm,objectiveValues.Ff.t_5,n_accuracy);
    Hx = getHessian(X_norm,objectiveValues.Fu_x.t_5,n_accuracy);
    Hy = getHessian(X_norm,objectiveValues.Fu_y.t_5,n_accuracy);
    Hz = getHessian(X_norm,objectiveValues.Fu_z.t_5,n_accuracy);
    ...
    for i=1:num_dim
        ...
        OF = p(i,1)*objectiveValues.Ff.t_5+p(i,2)*objectiveValues.Fu_x.t_5+p(i,3)*objectiveValues.Fu_y.t_5+p(i,4)*objectiveValues.Fu_z.t_5;
        H = getHessian(X_norm,OF,n_accuracy);
        [V,K] = eig(H);
        varNames={'Hessian','Eigenvectors','Eigenvalues'};
        T = table(H,V,K,'VariableNames',varNames);
        disp(T)
...
idealP = p(idealParam,:);
OF = idealP(1)*objectiveValues.Ff.t_5+idealP(2)*objectiveValues.Fu_x.t_5+idealP(3)*objectiveValues.Fu_y.t_5+idealP(4)*objectiveValues.Fu_z.t_5;
